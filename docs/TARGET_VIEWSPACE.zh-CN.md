...
现行候选策略为 `lazy_3d_shadow_prefilter_then_uniform_azimuth_v4`。旧 test2/test3 图片来自更早实现，不能用于判断本版输出；本版尚未生成新的真实房间渲染结果。
...
`viewspace_search.json` 记录各竖直平面的可行性、每次仰角和距离探测、拒绝原因、预算停止原因以及各水平桶裁剪前后数量；其中 `shadow_prefilter` 还记录粗走廊采样数、查询位置数、淘汰位置数和因此省去的完整可见性查询数。`target_view_evidence.json` 为每个候选记录水平角、仰角、距离、离地高度、可见率、填充率、composition 和所属水平桶。这些字段用于解释结果，不改变下游 K/R/T 格式。
