...
每段测试近端、中间、远端，再用广度优先队列二分：先探测各个独立自由区间，再逐层加密，避免先在第一个区间的近端深挖。预算允许时至少细分两层，再按合法性切换、投影边界变化、可见性变化细分，受最大深度与家具尺度归一化步长限制。太远导致目标过小的区间已由构图壳层外边界提前裁掉，最终评价仍重复检查一次作为数值保险。没有把可见性当成单调函数，也不声称有限测试发现了所有极窄可行区间。
...
现行候选策略为 `framing_shell_then_shadow_then_stratified_azimuth_v5`。旧 test2/test3 图片来自更早实现，不能用于判断本版输出；本版尚未生成新的真实房间渲染结果。
...
在昂贵的完整可见性评价之前，程序先定义一个按需计算的三维遮挡禁区。对被查询的位置 `P`，从 `P` 向扩展 capture box 的少量确定性表面点发射线段；这些线段是 `P` 到目标区域有限观察走廊的稀疏代理。若被目标以外场景几何挡住的比例已经很高，该**具体三维位置**直接淘汰，不再进行完整目标表面和 capture-box 射线检查。它不会因为茶几占据某个水平角就删除整个竖直平面：同一水平角中低位置可以被排除，高位置仍可进入后续筛选。这个禁区是懒计算并按位置缓存的，不会先体素化整个房间。

粗排除之后，水平角仍先做完整均匀扫描，距离区间使用二分式均匀细分；只有合法性变化或质量/可见性变化明显的区间继续加密。粗排除阈值必须低于最终 `min_capture_visibility_fraction`，因此它只负责删除明显无效区域，最终判断仍由完整构图、目标表面可见率和 capture-box 外部可见率完成。

完成 24 个水平角度后，第一轮在相邻角度之间全部补一个中点，相当于检查粗采样遗漏的角度。更多轮只在相邻端点至少一侧可行或可行性变化的位置继续补点。对最终被测水平角构造圆周 Voronoi 单元，把相邻合法单元合并为若干近似可行水平角区间。它是有限采样近似，不是连续区间的形式证明，极窄且没有命中任何探针的可行区仍可能遗漏。

方向给定后，由构图、房间边界和障碍盒先计算可搜索距离段；各段按广度优先测试近端、中点、远端并有限二分。每个方向最多 17 个距离点。只保留该方向中构图最好的点和距离最近的合法点，不再保留最远点。

候选点先通过有限竖直平面中的位置约束：离地高度、最大距离、允许相机区域及相机中心碰撞。这里的安全半径只用于相机位置与后续运镜碰撞，不代表画面遮挡。具体阈值由配置给出，不能把一次测试值写成算法常量。

候选裁剪不再机械沿完整圆周使用最初 24 个桶，而是先给每个近似可行水平角区间至少一个角度层，再按区间角宽成比例分配其余层，在每段内部等距放置层中心。候选映射到这些可行区间层后轮询保留，每层最多 6 个、全局最多 128 个；同层内仍以角度分散和单张质量决定保留顺序。这样不把预算浪费给已确认无合法点的空方向，也不会只向某个局部最高分附近加密。合法水平范围确实很窄时会如实报告较窄覆盖，少于最终预算时直接报错。

局部微调从不同水平桶选择最多 8 个种子，在世界 x/y/z 正负方向做两轮有限 pattern search。每个新位置仍需重新通过全部硬门槛。方向和位置查询分别受 600 与 4096 的总预算限制；局部微调共享位置预算。

`viewspace_search.json` 记录每个方向的构图壳层区间、减去 AABB 位置碰撞后的距离片段、各竖直平面的可行性、每次仰角和距离探测、预算停止原因、近似可行水平角区间以及分层中心；其中 `shadow_prefilter` 还记录粗走廊采样数、查询位置数、淘汰位置数和因此省去的完整可见性查询数。`target_view_evidence.json` 为每个候选记录水平角、仰角、距离、离地高度、可见率、填充率、composition 和所属水平层。这些字段用于解释结果，不改变下游 K/R/T 格式。
...
候选已经通过位置、视锥、目标可见率、capture 观察走廊和构图硬门槛。集合阶段先施加两项硬去重：任意一对机位都必须同时满足最小三维观察方向夹角和最小相机中心距离。这样一对近乎重合的机位不能再被其他相机对的平均多样性掩盖。
...
选择从多个分散起点开始，在不违反硬去重的候选中按水平覆盖、锚点角度互补、平均/最差构图和仰角差异逐步构成固定数量的集合。路径长度不再参与这一阶段，因而不能用短路线换取重复镜头。若在硬约束下凑不够 K 台，程序明确报告不足，而不是复制邻近方向。

集合固定后才计算机位间的 FreeGrid/A* 安全连接，并对不超过 12 台的集合用动态规划求精确最短开放访问顺序。路径只负责可达性，并在选择质量完全相同时作为次级判据。沿途仍按固定步长复核 capture 视锥与遮挡。
