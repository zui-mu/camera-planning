...
2026-09-13 的构图壳层实现依据标准针孔投影 `u=fx Xc/Zc+cx, v=fy Yc/Zc+cy`，把默认的目标家具角点（或消融实验中的 capture-box 角点）画面边界约束转成方向相关近界，并用一维有界求解得到最小投影尺寸对应的远界。文献只支持“先形成满足光学/几何约束的有限视点区域，再做遮挡与集合选择”这一设计原则；方向相关壳层公式是本仓库针对固定 look-at 区域的直接推导，不应写成复现了某篇论文的专有公式。
